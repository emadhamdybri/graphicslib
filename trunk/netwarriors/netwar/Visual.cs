...
namespace netwar
{
    ...
    class Visual : GameWindow
    {
        ...
        public Visual(int width, int height, GraphicsMode mode, GameWindowFlags options)
            : base(width, height, mode, "NetWarriors", options)
        {
        }

        public void LoadGameWorld( PortalWorld w )
        {
            World = new PortalWorld();
            World.MapObjects = w.MapObjects;
            World.MapAttributes = w.MapAttributes;

            foreach (CellGroup group in w.CellGroups)
            {
